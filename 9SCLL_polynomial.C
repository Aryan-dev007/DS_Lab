...
NODE *read_poly(NODE *head) {
	...
	do {
		printf("Enter coeff: ");
		scanf("%d", &cf);
		printf("\nEnter powers for x, y, z (0 indicate no term):  ");
...
void evaluate(NODE *h1) {
	...
	while (h1 -> link != head) {
		h1 = h1 -> link;
		result = result + (head -> cf* pow(x,h1->px)*pow(y,h1->py)*pow(z, h1->pz));
		h1 = h1 -> link;
	}
